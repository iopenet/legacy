...
define( '_INSTALL_L0', 'XCL 2.3.1 インストールウィザードへようこそ' );
define( '_INSTALL_L168', 'XCL の動作には PHP7.2 以降が必要です' );
...
define( '_INSTALL_L11', 'XOOPSCubeのディレクトリへのパス：' );
define( '_INSTALL_L12', 'XOOPSCubeへのURL：' );
...
define( '_INSTALL_L19', '%s はXOOPSCubeが使用するデータベースの名前です。' );
define( '_INSTALL_L20', '%s はXOOPSCubeが使用する各データベーステーブルに付加されるprefix（接頭語）です。prefixを付加することで、既存テーブルとのテーブル名称の重複を防ぎます。' );
...
define( '_INSTALL_L55', 'XOOPSCubeへのパス' );
define( '_INSTALL_L59', '　XOOPSCubeが設置されているディレクトリへのフルパスを入力してください。<br>　末尾には「/」を付加しないでください。' );
...
define( '_INSTALL_L56', 'XOOPSCubeへのURL' );
define( '_INSTALL_L58', '　XOOPSCubeにアクセスするURLを入力してください。<br>　末尾には「/」を付加しないでください。' );
...
define( '_INSTALL_L35', "インストール中にエラーが発生した場合は<a href='http://xoopscube.jp/'>XOOPSCube日本語サイト</a>のサポートフォーラムをご利用ください。" );
...
define( '_INSTALL_L46', 'XOOPSCubeのモジュールが正常に動作するには、下記のファイルがサーバにより書き込み可能になっている必要があります。（UNIX/LINUXサーバをご使用の場合、各ファイルのパーミッションを666または777に設定してください。）' );
...
define( '_INSTALL_L141', 'インストーラは今から、XOOPSCubeで動くように各モジュールをアップデートします。<br>XOOPSCubeのパッケージに含まれるすべてのファイルがサーバにアップロードされているか確認してください。<br>これが完了するまでには、しばらく時間が掛かるかもしれません。' );
define( '_INSTALL_L142', 'モジュールのアップデート中…' );
define( '_INSTALL_L143', 'The installer will now update configuration data of XOOPS2 to be used with XOOPSCube.' );    //[MADA]
...
define( '_INSTALL_L149', 'インストーラは今から、XOOPSCubeで使えるようにXOOPS 1.3.xのコメントポストをアップデートします。<br>これが完了するまでには、しばらく時間が掛かるかもしれません。' );
define( '_INSTALL_L150', 'インストーラは今から、XOOPSCubeで使えるように顔アイコンとユーザランキングの画像をアップデートします。<br>これが完了するまでには、しばらく時間が掛かるかもしれません。' );
define( '_INSTALL_L151', 'インストーラは今から、XOOPSCubeで使えるようにユーザアバターの画像をアップデートします。<br>これが完了するまでには、しばらく時間が掛かるかもしれません。' );
